...
class TestClass extends Base {
  ...
  render() {
    ...
    this.style.backgroundColor = `rgb(${Number(!this.isVisible) * 255},${
      Number(this.isVisible) * 255
    },0)`;
  }
}
...
let state2 = new State(Ok("Testing Attention Please"));

inst.attachStateToProp("innerHTML", state2);

setTimeout(() => {
  state2.set(Ok("Testing something else"));
}, 2000);
...
setTimeout(() => {
  ...
  setTimeout(() => {
    state3.set(Ok(AccessTypes.read));
    setTimeout(() => {
      state3.set(Err({ reason: "Test Error" }));
    }, 2000);
  }, 2000);
}, 2000);
...
let connectTestButton = connectTest.appendChild(
...
let connectTestContainer = connectTest.appendChild(
  document.createElement("div")
);
connectTestContainer.style.display = "flex";
connectTestContainer.style.flexDirection = "column";

let attributeTest = new State(Ok("TestAttribute"));
// setInterval(() => {
//   attributeTest.set(Ok(attributeTest.get().unwrap + "a"));
// }, 1000);
...
for (let i = 0; i <= 19; i++) {
  ...
  inst.attachStateToAttribute("test", attributeTest);
}

let state = new State(Ok(AccessTypes.write));
...
let observerTestButtonObs = observerTest.appendChild(
  document.createElement("button")
);
...
let observerTestButtonConn = observerTest.appendChild(
  document.createElement("button")
);
...
let observerTestContainer = observerTest.appendChild(
  document.createElement("div")
);
